...
if(!empty($cbsd_version)):
	...
	exec("/bin/echo; /usr/local/bin/cbsd show_profile_list search_profile=vm-\*-cloud show_cloud=1 show_jail=1 uniq=1 display=path header=0 | /usr/bin/sed -e 's:.conf::g' -e \"s:{$workdir}/etc/defaults/vm-::g\" | /usr/bin/tr -s ' ' '\n';",$profileinfo);
